...
----mongodb
wget -nd https://fastdl.mongodb.org/osx/mongodb-osx-ssl-x86_64-3.6.2.tgz -P /usr/local/
cd /usr/local/
tar zxf mongodb-osx-ssl-x86_64-3.6.2.tgz
sudo ln -s mongodb-osx-x86_64-3.6.2 ./mongodb
vim ~/.bash_profile
...
# 进入mongo shell之后
use admin
db.createUser({user:'Mike', pwd:'123456', roles: [{role:'userAdminAnyDatabase', db: 'admin'}]})
# 退出，重启服务
mongod --auth --config /usr/local/mongo/conf/mongodb.conf
# 再次进入mongo shell
mongo -u Mike -p 123456
# 这时没有错误了
```
